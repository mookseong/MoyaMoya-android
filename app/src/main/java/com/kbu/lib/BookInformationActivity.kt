...
class BookInformationActivity : BaseActivity(R.layout.activity_book_information) {
    ...
    override fun set() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            val TextviewId = arrayListOf<TextView>(Title_information, DCC_information, number_information, weiter_information, text_information)
            val titlelist = arrayListOf("DDC", "청구기호", "서명/저자", "발행사항")
            val asysnc = GlobalScope.async(Dispatchers.IO) {
                val DATA_elementstitle = Jsoup.connect(libURL + URL).get().select("div[class=col-md-10 detail-table-right]")
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            val asysnc =GlobalScope.async(Dispatchers.IO) {
                val possess_information_elements =
                    Jsoup.connect(libURL + URL).get().select("div[class=sponge-guide-Box-table sponge-detail-table]").select("table[class=table-striped sponge-table-default]").select("tbody tr")
